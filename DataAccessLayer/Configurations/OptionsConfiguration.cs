...
namespace DataAccessLayer.Configurations
{
    public partial class OptionsConfiguration : IEntityTypeConfiguration<Options>
    {
        public void Configure(EntityTypeBuilder<Options> builder)
        {
            ...
            builder.HasOne<ProductDetails>(c => c.ProductDetails).WithMany(c => c.Options).HasForeignKey(c => c.IDProductDetails).OnDelete(DeleteBehavior.Cascade);
            builder.HasOne<Colors>(c => c.Colors).WithMany(c => c.Options).HasForeignKey(c => c.IDColor).OnDelete(DeleteBehavior.Cascade);
            builder.HasOne<Sizes>(c => c.Sizes).WithMany(c => c.Options).HasForeignKey(c => c.IDSize).OnDelete(DeleteBehavior.Cascade);
